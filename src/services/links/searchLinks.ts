...
export async function nameSearchedInDb(searchName: string): Promise<{ id: number; name: string }[]> {
 if(!searchName) {
  ...
  try {
      ...
      //   console.error("Réponse de l'API error:", data.message);
        ...
        throw new Error(data.message || "Erreur lors de la récupération des noms");
      }
console.log("data", data);
      const results = data.map((item: { id: number; name: string ; avatar:string}) => ({
        id: item.id,
        name: item.name,
        avatar: item.avatar || "", 
      }));
