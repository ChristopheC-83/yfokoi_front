...
import type { activListShare, ShareToCreate } from "@/types/ListShare";
...
export async function createShareFromApi(share: ShareToCreate): Promise<activListShare> {
  const res = await fetchSharesWithAuth(`${URL_API}/api_shares/createShare`, {
    method: "POST",
...
export async function updateShareFromApi(
  listId: number,
  userId: number,
  access: number
): Promise<void> {
...
export async function removeShareFromApi(
  listId: number,
  userId: number
): Promise<void> {
